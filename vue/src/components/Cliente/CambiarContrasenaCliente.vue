...
export default {
  name: "CambiarContrasenalCliente",
  components: {
    NavCliente
  },
  data() {
  ...
  methods: {
    submitForm() {
      if (this.newPassword !== this.confirmPassword) {
        ...
        this.$router.push("/cambiar-contrasena/cliente");
        setTimeout(() => {
      ...
      ClienteService.changePassword(clienteId, cambioContraseñaData)
        .then(() => {
        ...
        .catch((error) => {
          console.log("Error al cambiar la contraseña del cliente: ", error);
          store.commit("setNotification", "error");
